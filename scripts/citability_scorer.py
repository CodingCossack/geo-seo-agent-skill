...
Uses practical heuristics for answer-ready passages:
- usually compact enough to quote cleanly
- self-contained enough to stand without surrounding context
- specific enough to contain verifiable facts or examples
- structured enough to expose a direct answer early
"""
...
def score_passage(text: str, heading: Optional[str] = None) -> dict:
    ...
    # Preferred word-count band for compact, self-contained passages
    if 110 <= word_count <= 220:
        sc_score += 10
    elif 90 <= word_count <= 260:
        sc_score += 7
    elif 70 <= word_count <= 320:
        sc_score += 4
...
def analyze_page_citability(url: str) -> dict:
    ...
    if scored_blocks:
        ...
        # Count passages in the preferred compact range
        optimal_count = sum(
            1 for b in scored_blocks if 110 <= b["word_count"] <= 220
        )
    else:
        avg_score = 0
        top_blocks = []
